...
contributor adding a new model page declares the shape of the parameters their
interactive regions accept, and the type checker verifies that the components
consuming those parameters receive what they expect. TypeScript's inference
handles most internal logic without explicit annotation, concentrating the
annotation burden where it is most useful. Astro compiles TypeScript without
