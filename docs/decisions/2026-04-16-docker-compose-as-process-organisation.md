...
not only starting multiple processes but doing so in a reproducible way, in the
correct order, with each process restarted automatically on failure, connected
...
restarting them on failure, and routing their output to the centralised journal.
Rejected because unit files live on the server's filesystem rather than in the
repository, separating the deployment configuration from the code it governs.
Systemd also provides no isolation between components: they share the host's
installed packages and runtime environment, meaning dependency conflicts between
components are the operator's problem to manage. The reproducibility guarantee
that Docker provides, the same environment on every machine that runs the
system, is absent. Systemd remains an appropriate mechanism for starting the
...
Procfile runners are development tools without production-grade supervision:
restart-on-failure behaviour is rudimentary, dependency ordering between
processes is not supported, and the tooling is not designed for long-running
unattended operation. The project does not maintain a Procfile. Contributors who
...
its complete environment, meaning the same image produces the same runtime on a
development machine and on the production server. Dependency conflicts between
components are contained: each container has its own filesystem and installed
packages. The compose file is committed to the repository alongside the code it
describes, making the system's composition visible to contributors and versioned
with the rest of the project. Docker Compose creates a private virtual network
shared by all containers, on which each container is reachable by its service
name, providing the network layer that the communication decision will build on
top of. Restart policies defined in the compose file ensure that a crashed
component is brought back without operator intervention. Dependency ordering,
