...
document for a specific quantitative model, containing prose, mathematical
proofs, and embedded interactive elements such as parameter controls and charts.
The page is a document first. Its primary content is written and authored. The
interactive elements are embedded within that document and serve to illustrate
and extend the written argument. The framework decision is made against this
primary artifact. At a future stage, when enough model pages exist to cover
meaningfully distinct areas of finance, a synthesis interface combining multiple
models into a unified analytical session becomes relevant. That interface is
structurally different from a model page: it would be an application rather than
a document, its structure is constructed by the user at runtime rather than
authored by contributors, and its inter-component state relationships may be
...
time and interactive regions implemented as React components hydrated as Astro
...
React applications in which some components happen to be static, where paralx's
model pages are documents in which some regions happen to be interactive.
Next.js client components hydrate on page load regardless of whether they are
visible to the user, meaning a long model page with multiple charts downloads
JavaScript for all charts immediately. Astro's per-island hydration scheduling,
which can defer hydration until a component enters the viewport, is a better fit
for long content pages with multiple embedded interactive elements. Next.js
defaults toward Vercel deployment and its documentation and advanced features
...
default and requires explicit opt-in for JavaScript. Interactive islands are
designated regions within an otherwise static document, which is exactly the
structure the hybrid rendering ADR describes. The framework was built in direct
response to the industry habit of using application frameworks for content that
is primarily documentary, and paralx's model pages are precisely the content
Astro was designed for. A model page with multiple charts does not download or
initialize the JavaScript for charts the user has not yet scrolled to. For users
on mobile connections encountering the project cold, this reduces the initial
JavaScript weight and improves time to interactivity. Astro supports React as a
component library for islands natively, meaning the React ADR's choice is not
compromised. Interactive regions are written as React components with access to
the full React ecosystem. Static output is Astro's default rather than a mode
opted into by configuring away from a server-oriented default. The build
produces a folder of HTML files and assets. The Go server serves them. No
JavaScript runtime runs in production, satisfying the build-time generation ADR
without requiring deliberate resistance to framework defaults. The framework's
...
The Go server's role does not change in either case.
