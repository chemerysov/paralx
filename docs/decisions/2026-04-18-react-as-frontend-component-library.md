...
decision of the component model. The project's interactive regions are
chart-heavy. Every model page contains parameter controls whose changes are
reflected in data visualizations. The project intends to reach retail investors
...
smaller than equivalent React bundles, which is directly relevant to the mobile
performance requirement. Svelte's built-in animation and transition primitives
are well-suited to data-driven chart transitions. Its origins in interactive
data journalism make it conceptually well-aligned with the project's
visualization requirements. Rejected because the chart library ecosystem is
materially thinner than React's: the options for financial chart types are
fewer, and the most capable libraries require more manual integration than their
React equivalents. The contributor pool is also significantly smaller than
React's, which is a concrete cost for an open source project. The mobile bundle
...
ecosystem is not mature: chart library coverage is thin, the contributor pool is
...
predominantly charts. The React ecosystem contains more high-quality financial
chart libraries than any alternative: Recharts, Nivo, and Visx are built
natively for React's component model, and Plotly.js and ECharts, both of which
cover the financial chart types the project requires, have mature React
wrappers. The available options in other ecosystems are either fewer, less
mature, or require more manual integration work. For a project where chart
quality is a core product requirement, this advantage is decisive. React's
secondary advantage is contributor accessibility. React is the most widely known
JavaScript component model. The bundle size disadvantage relative to Svelte is
real. React's runtime, including the virtual DOM reconciler, is approximately 40
to 45 kilobytes compressed, a fixed overhead that Svelte does not incur. This
cost is accepted on the grounds that the project's other mobile performance
levers, aggressive code splitting so that only the JavaScript needed for the
current page is downloaded, lazy hydration of chart components that are not
immediately visible, and the pre-rendered static HTML that is visible before any
JavaScript loads, are more impactful than the component model runtime size. The
...
React-based frameworks, which are the most mature and most documented options
...
re-render model, in which component functions re-run on state changes, requires
...
pre-rendered static HTML of the content regions, and what the production
